...
Vector gd_maxDiff(const ContinuousFunctions& c, const Vector& x0, double& maxDiff, const double alpha,
  ...
  maxDiff = 0;
    ...
    gold = g;
		if (verbosity > 0)
			printf("numIter: %d, alpha: %e, ObjVal: %e, OptCond: %e, NormDiff:  %e\n", funcEval, alpha, f, gnorm, gdiff);
	}
	return x;
}
}
