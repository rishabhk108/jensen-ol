...
#include "ProxL2LogisticRegression.h"
#include "../../optimization/contAlgorithms/contAlgorithms.h"
#include "../../optimization/contFunctions/L2LogisticLossProx.h"
...
ProxL2LogisticRegression<Feature>::ProxL2LogisticRegression(vector<Feature>& trainFeatures, Vector& y, int m, int n,
                                                    double lambda, double updatelambda, int algtype, int update_algtype, int maxIter,
                                                    double LR, double eps, int miniBatch, int lbfgsMemory) :
                                                    Classifiers<Feature>(m, n), trainFeatures(trainFeatures), y(y),
                                                    lambda(lambda), updatelambda(updatelambda), algtype(algtype), update_algtype(update_algtype), maxIter(maxIter),
                                                    LR(LR), eps(eps), miniBatch(miniBatch), lbfgsMemory(lbfgsMemory) {}

template <class Feature>
ProxL2LogisticRegression<Feature>::ProxL2LogisticRegression(const ProxL2LogisticRegression<Feature>& c) : Classifiers<Feature>(m, n),
	trainFeatures(c.trainFeatures), y(c.y), lambda(c.lambda), updatelambda(c.updatelambda), algtype(c.algtype), update_algtype(c.update_algtype),
	maxIter(c.maxIter), LR(c.LR), eps(c.eps), miniBatch(c.miniBatch), lbfgsMemory(c.lbfgsMemory) {
}

template <class Feature>
ProxL2LogisticRegression<Feature>::~ProxL2LogisticRegression(){
}

template <class Feature>
void ProxL2LogisticRegression<Feature>::train() {
...
template <class Feature>
void ProxL2LogisticRegression<Feature>::update(vector<Feature>& incTrainFeatures, Vector& incy)
{
  // Update L2 regularized logistic regression
    Vector wnew = w;
    minimize(incTrainFeatures, incy, updatelambda, maxIter, update_algtype, w, wnew, LR);
    w = wnew;
}

template <class Feature>
Vector ProxL2LogisticRegression<Feature>::get_weights(){
  // get weights
    return w;
}

template <class Feature>
void ProxL2LogisticRegression<Feature>::initialize_weights(Vector& wprime){
    w = wprime;
}

template <class Feature>
void ProxL2LogisticRegression<Feature>::minimize(vector<Feature>& trainFeatures, Vector& labels, double lam,
  int maxiter, int algotype, Vector& x0, Vector& x, double alpha)
{
  x = Vector(x0.size(), 0);
  L2LogisticLossProx<Feature> ll(m, trainFeatures, labels, x0, lam);
...
// save the model
template <class Feature>
int ProxL2LogisticRegression<Feature>::saveModel(char* model){
...
template <class Feature>
int ProxL2LogisticRegression<Feature>::loadModel(char* model){
...
template <class Feature>
double ProxL2LogisticRegression<Feature>::predict(const Feature& testFeature, double& val){
...
template <class Feature>
double ProxL2LogisticRegression<Feature>::predict(const Feature& testFeature){
	// the assumption here is that train and test datasets have the same number of features
	double val;
	val = featureProductCheck(w, testFeature);
	double argval = 0;
...
// classification.
template <class Feature>
void ProxL2LogisticRegression<Feature>::predictProbability(const Feature& testFeature, Vector& prob){
	// the assumption here is that train and test datasets have the same number of features
	prob = Vector(2, 0);
	double val;
	val = featureProductCheck(w, testFeature);
...
template class ProxL2LogisticRegression<SparseFeature>;
template class ProxL2LogisticRegression<DenseFeature>;
}
